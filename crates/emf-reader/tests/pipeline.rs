use dft2dxf_testkit::{build_line_emf, build_minimal_dft, MinimalDftSpec};
...
fn parses_rectangle_emf_records() {
  let emf = build_line_emf(0, 0, 100, 50);
...
fn replays_rectangle_to_drawing_ir() {
  let emf = build_line_emf(0, 0, 100, 50);
  let doc = EmfDocument::parse(&emf, DEFAULT_MAX_RECORD_COUNT, DEFAULT_MAX_RECORD_SIZE).unwrap();
  let drawing = replay_to_drawing(&doc, Some(1), Some("Sheet1".to_string()), Some(297.0), Some(210.0));
...
fn end_to_end_synthetic_pipeline() {
  ...
  let emf = build_line_emf(5, 5, 95, 45);
  ...
  let emf_doc =
    EmfDocument::parse(&extracted.data, DEFAULT_MAX_RECORD_COUNT, DEFAULT_MAX_RECORD_SIZE)
      .unwrap();
